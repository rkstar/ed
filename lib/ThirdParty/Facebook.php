...
/*
 * 
 * MODIFIED by DAVID FUDGE 2.18.2011
 *
 * we're always using constants defined in the ed config
 * so why not just make this sdk aware of them?
 *
 *
 ...
 *
 *
 * new constructor below...
 */
 public function __construct()
 {
 	$this->setAppId(FACEBOOK_APP_ID);
 	$this->setApiSecret(FACEBOOK_APP_SECRET_KEY);
 	$this->setCookieSupport(FACEBOOK_USE_COOKIES);
 	$this->setBaseDomain(DOMAIN);
 	//
 	// ignore the file upload support...
 	// we'll enable that on a per/use basis.
 	//
 }
  ...
  protected static function errorLog($msg) {
    ...
    // @codeCoverageIgnoreStart
/*
 * commented by Dave Fudge...
 * this is NOT the correct use of this...
...
*/
  ...
  //
  // MODIFIED BY DAVID FUDGE 2.14.2011
  //
  // i've added this static function because I think it will be useful
  // when dealing with both PHP and Javascript SDKs.
  public static function decodeSignedRequest( $request )
  {
  	list($encoded_signature, $encoded_data) = explode(".", $request, 2);
  	$signature	= self::base64UrlDecode($encoded_signature);
  	$userdata	= json_decode(self::base64UrlDecode($encoded_data), true);
  	if( strtoupper($userdata["algorithm"]) != "HMAC-SHA256" ) { return null; }

  	$valid_signature = hash_hmac("sha256", $encoded_data, FACEBOOK_APP_SECRET_KEY, true);
  	return ($signature != $valid_signature) ? null : $userdata;
  }
  //
  //
  // end modified...
